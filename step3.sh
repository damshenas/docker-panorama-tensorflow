...
pip3 install -U pip six 'numpy<1.19.0' wheel setuptools mock 'future>=0.17.1' 'gast==0.3.3' typing_extensions h5py
...
bazel build --config=opt --config=noaws --config=nogcp --config=nohdfs --config=nonccl --local_cpu_resources=HOST_CPUS*0.25 --local_ram_resources=HOST_RAM*0.5 //tensorflow/tools/pip_package:build_pip_package
            # --config=cuda \
            # --config=noaws \
...
#pip3 install wheel/tensorflow_pkg/tensorflow-2.3.0-cp36-cp36m-linux_aarch64.whl
pip3 install wheel/tensorflow_pkg/tensorflow-2.3.0-*.whl
